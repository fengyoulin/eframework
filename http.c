...
int ef_curl_set_header(ef_curl *cu,char *header,char *content){
    ...
    if(!ef_stringtab_set(cu->req->headers,header,strlen(header),content,strlen(content))){
        return -1;
    }
    return 0;
}
...
int ef_curl_set_request_body(ef_curl *cu,char *key,char *val){
    ...
    if(!ef_stringtab_set(cu->req->request_body,key,strlen(key),val,strlen(val))){
        return -1;
    }
    return 0;
}
...
int http_build_request_header(ef_curl_request *req,char *buf,size_t cap){
    ...
    v = ef_stringtab_find(req->headers,k->str,k->len);
    ...
    *pos++ = ':';
    ...
    strcat(pos,"\r\n");
    pos += 2;
    res = ef_stringtab_remove(req->headers,k->str,k->len);
    if(res < 0){
...
int ef_curl_read_response(ef_curl *cu,char *buf,size_t len){
    ...
    ef_string_t *v = ef_stringtab_find(cu->resp->headers,headerName,strlen(headerName));
...
int http_buffer_resp_body(int fd,ef_curl_response *resp){
    ...
    ef_string_t *v = ef_stringtab_find(resp->headers,headerName,strlen(headerName));
    if(v){
...
int http_parse_resp_header(ef_curl_response *resp,char *start,char *bufend){
        ...
        *end = '\0';
        ...
        res = ef_stringtab_set(resp->headers,header,pos-header,val,end-val);
